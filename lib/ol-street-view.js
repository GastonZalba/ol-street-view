...
import { XYZ, Vector as Vector$1 } from 'ol/source';
...
class StreetView {
  ...
  _prepareLayers() {
    var calculatePegmanIconOffset = () => {
      var heading = this._pegmanHeading;
    ...
    this._streetViewXyzLayer = new Tile({
      zIndex: 10,
      source: new XYZ({
  ...
  _addMapInteractions() {
    var translatePegmanHandler = evt => {
  ...
  _prepareLayout() {
    ...
    var addStreetViewHtml = () => {
      ...
      this.exitControlUI.title = this._i18n.exitView; //this.exitControlUI.index = 1;
      ...
      this.mapContainer.className = 'ol-street-view--transitions'; // Move the map element (viewport) inside a new container
  ...
  _createMapControls() {
    ...
    var addPegmanInteraction = () => {
      ...
      var onMouseMove = e => {
        ...
        if (e.pageX < oldPosX) {
          ...
          this.pegmanDraggable.classList.remove('ol-street-view--right'); // Right
      ...
      var terminateDragging = () => {
        ...
        this.pegmanDraggable.removeAttribute('data-y'); // Remove Dropzone Feedback

        this.viewport.classList.remove('ol-street-view--drop-active', 'ol-street-view--drop-target');
      ...
      interact('.ol-street-view--draggable').draggable({
        ...
        onend: e => {
          var viewportOffset = this.mapContainer.getBoundingClientRect(); // To compensate if the map is not 100%  width of the browser

          var mapDistanceX = viewportOffset.left;
          var mapDistanceY = viewportOffset.top; // Compensate cursor offset

          var location = this.map.getCoordinateFromPixel([e.client.x - mapDistanceX, e.client.y - mapDistanceY + this.pegmanDraggable.clientHeight - 10]);
    ...
    var addPegmanControl = () => {
      this.pegmanDivControl = document.createElement('div');
      this.pegmanDivControl.id = 'ol-street-view--pegman-button-div';
      this.pegmanDivControl.className = "ol-control ol-street-view--".concat(this.options.size, "-btn");
    ...
    var addSizeTogglerControl = () => {
      ...
      togglerBtn.onclick = () => {
        ...
        if (document.body.classList.contains(compactClass)) {
          // Minimized
          togglerBtn.title = this._i18n.expand; // Store height for later
          ...
          // Expanded
          togglerBtn.title = this._i18n.minimize;
          if (this._lastHeight) this.viewport.style.height = this._lastHeight;
  ...
  _loadStreetView() {
    return __awaiter(this, void 0, void 0, function* () {
      var loader = new Loader(this.options.apiKey, {
  ...
  _updateStreetViewPosition(coords) {
    var latLon = transform(coords, this.view.getProjection(), 'EPSG:4326').reverse();
    var latLonGoogle = {
      lat: latLon[0],
      lng: latLon[1]
    };
    var streetViewService = new google.maps.StreetViewService();
    streetViewService.getPanoramaByLocation(latLonGoogle, SV_MAX_DISTANCE_METERS, (_, status) => {
  ...
  _initStreetView() {
    ...
    this._panorama.addListener('position_changed', () => {
      var position = this._panorama.getPosition();
  ...
  _addClickListener() {
    var clickListener = evt => {
      var position = this.map.getCoordinateFromPixel(evt.pixel);
  ...
  hideStreetView() {
    ...
    unByKey(this._keyClickOnMap); // Maybe, exit fullscreen
